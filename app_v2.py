...
tab1, tab2, tab3 = st.tabs(["📊 Panorama", "🔎 Evolución", "🧭 Clasificación"])
...
with tab2:
    ...
    st.subheader("4. Causas con cambios más destacados")
    st.caption("Comparación directa de las variaciones más positivas y negativas.")

    col_a, col_b = st.columns(2)

    with col_a:
        inc = (
            filtered.nlargest(min(10, len(filtered)), "variacion_pp")
            .sort_values("variacion_pp")
        )
        fig_inc = px.bar(
            inc,
            x="variacion_pp",
            ...
            text="variacion_pp",
            labels={
                "variacion_pp": "Variación (puntos porcentuales)",
                "causa_original": "Causa",
            },
            title="Mayores aumentos",
        )
        fig_inc.update_traces(texttemplate="%{text:.2f} pp", textposition="outside")
        fig_inc.update_layout(height=560, margin=dict(l=20, r=70, t=70, b=20))
        st.plotly_chart(fig_inc, use_container_width=True)

    with col_b:
        dec = (
            filtered.nsmallest(min(10, len(filtered)), "variacion_pp")
            .sort_values("variacion_pp", ascending=False)
        )
        fig_dec = px.bar(
            dec,
            x="variacion_pp",
            ...
            title="Mayores descensos",
        )
        fig_dec.update_traces(texttemplate="%{text:.2f} pp", textposition="outside")
        fig_dec.update_layout(height=560, margin=dict(l=20, r=70, t=70, b=20))
